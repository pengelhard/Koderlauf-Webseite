...
test("WhatsApp-Text hat https-URL in eigener Zeile", () => {
  const url = "https://test.koderlauf.de/fassjagd/tv-1860-gunzenhausen?v=3";
  const text = whatsappText(club, url);
  assert.match(text, /\nhttps:\/\/test\.koderlauf\.de\/fassjagd\/tv-1860-gunzenhausen\?v=3$/);
  assert.equal(text.includes("Platz 1"), true);
  assert.equal(text.split("\n").length, 2);
});

test("Hausherr-WhatsApp-Text ebenfalls mit URL-Zeile", () => {
  const url = "https://test.koderlauf.de/fassjagd/sv-obermoegersheim?v=3";
...
test("Team-Share- und OG-Image-URLs sind absolut und cache-bustend", () => {
  assert.equal(
    ...
    "https://test.koderlauf.de/fassjagd/sv-obermoegersheim?v=3",
  );
  assert.equal(
    teamOgImageUrl("https://test.koderlauf.de", "sv-obermoegersheim"),
    "https://test.koderlauf.de/fassjagd/sv-obermoegersheim/opengraph-image?v=3",
  );
});
